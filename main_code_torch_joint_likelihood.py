...
zeta_l = tch.abs(y_l + tch.normal(0.0, 0.05, (1, n_l)).squeeze(0))
h_l = tch.zeros(n_l)
h_f = tch.zeros(n_f)
...
def log_prob_fn_lab(lambda_h, lambda_l, h_l, zeta_l):
    ...
    # p(h_l | zeta_l, a_l)
    log_likelihood2 = n_l / 2 * tch.log(lambda_h) - n_l / 2 * tch.logdet(K_h(a_l, zeta_l, s_l, inv = False) + tch.diag(tch.ones(n_l) * val_nugget)) - lambda_h / 2 * (h_l - func_mean(a_l, zeta_l)) @ K_h(a_l, zeta_l, s_l, inv = True) @ (h_l - func_mean(a_l, zeta_l)).T
...
def log_prob_fn_field(lambda_h, lambda_delta, h_l, theta, zeta_l, h_f, eta_f, delta_f):
    ...
    log_likelihood2 = n_f / 2 * tch.log(lambda_h) - n_f / 2 * tch.logdet(K_hat_h_f  + tch.diag(tch.ones(n_f) * val_nugget)) - lambda_h / 2 * (h_f - h_hat_f) @ K_hat_h_f_inv @ (h_f - h_hat_f).T
    ...
    log_likelihood3 = -n_f / 2 * tch.logdet(K_hat_eta_f + tch.diag(tch.ones(n_f) * val_nugget)) - lambda_eta / 2 * (eta_f - eta_hat_f) @ K_hat_eta_f_inv @ (eta_f - eta_hat_f).T

    # p(delta_f | x_f)
    log_likelihood4 = n_f / 2 * tch.log(lambda_delta) - tch.logdet(K_delta(x_f, inv = False) + tch.diag(tch.ones(n_f) * val_nugget)) - lambda_delta / 2 * delta_f @ K_delta(x_f, inv = True) @ delta_f.T
...
optimizer = tch.optim.Adam([lambda_h], lr = 0.1)
